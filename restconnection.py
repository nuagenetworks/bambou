...
from restnuage.httpexceptions import HTTPTimeoutException
...
class RESTConnection(object):
    ...
    def _is_response_success(self, response, should_post=False):
        ...
        if status_code == HTTP_CODE_ZERO:
            print "RESTConnection: Connection error with code 0. Sending NURESTConnectionFailureNotification notification and exiting."
            self._print_information(error_name, error_description)
            return False

        print "RESTConnection: Report this error, because this should not happen: [%s] %s" % (status_code, response.data)
        return False
